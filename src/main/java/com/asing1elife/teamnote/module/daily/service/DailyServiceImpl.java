...
public class DailyServiceImpl extends BaseService<DailyModel, DailyRepository> {
    ...
    @Transactional
    public Map<Integer, List<DailyModel>> getDailiesByYear() {
        // 获取或保存日志及日志记录
        getOrSaveDailyAndRecord();

        // 获取所有日志
        return generateDailiesOfYear();
    }

    /**
     * 获取或保存日志及日志记录
     */
    public DailyRecordModel getOrSaveDailyAndRecord() {
        // 尝试获取当前年当前月的日志
        DailyModel currentMonthDaily = repository.getByYearAndMonth(DateUtil.getCurrentYear(), DateUtil.getCurrentMonth());

        // 没有则创建
        if (currentMonthDaily == null) {
            currentMonthDaily = saveDaily();
        }
    ...
    private Map<Integer, List<DailyModel>> generateDailiesOfYear() {
        Map<Integer, List<DailyModel>> dailyMap = Maps.newHashMap();

        // 获取按照创建时间排序的日志列表
        List<DailyModel> dailies = getDailiesOrderByYearAndMonth();
     ...
     * 获取按照创建时间排序的日志列表
     */
    private List<DailyModel> getDailiesOrderByYearAndMonth() {
        return super.findAll(new Sort(Sort.Direction.ASC, "year", "month"));
    }

    /**
     * 保存日志
     */
    private DailyModel saveDaily() {
